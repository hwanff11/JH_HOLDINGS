...
현재 배포·모의운용·실거래 상태는 [`../CURRENT_WORK.md`](../CURRENT_WORK.md)에서만 확인합니다.
...
핵심 철학은 **빠른 위험축소·느린 위험확대**입니다. 위험이 커질 때는 월중에도 줄일 수 있지만, 한 번 줄인 위험을 같은 달 안에서 쉽게 다시 늘리지 않습니다.
...
1. 완결 거래일 데이터로 목표비중과 전략 계산차수를 저장
2. 새 목표가 생기면 **다음 미국 거래시간이 시작된 뒤 최신 가격으로 목표수량을 한 번 고정**
3. 현재 보유·미체결 주문과 목표수량 차이를 계산
4. 줄여야 할 수량은 위험축소 매도 우선
5. 매도 종료·원장 반영·계좌 대조가 끝나야 매수 허용
6. 매수가 필요하면 Telegram 주문 묶음 검토로 운영자에게 제시

주문은 장전·정규장·장후에 허용됩니다. 정규장에서도 신규 매수 잠금과 2단계 승인은 유지됩니다. Toss 현재가에 시각이 없거나 5분보다 오래되면 구 가격으로 주문하지 않고 새 가격을 확인할 때까지 중단합니다.
...
flowchart TD
    ...
    D --> E{"현재 보유와 목표 비교"}
    E -->|초과 보유| F["위험축소 매도 우선"]
    F --> G["체결 반영·계좌 대조"]
    E -->|부족 보유| H["매수 후보"]
    G --> H
    H --> I["오늘 주문 한번에 검토"]
    I --> J["전체 매수금액·주문시간·계좌 대조 사전점검"]
    J --> K["운영자: N건 순차 실행 최종 승인"]
    K --> L["종목별 독립 주문"]
    L --> M["부분/전량 체결 감시"]
    M --> N{"원장 일치?"}
    N -->|예| A
    N -->|아니오| O["안전정지 · 신규 매수 차단"]
```

### 운영자가 실제로 누르는 흐름

평소에는 `/dashboard` → **`오늘 주문 확인`**을 사용합니다.
...
사용자 화면은 주문 묶음 검토로 단순화했지만 내부에서는 각 매수 신호의 검토 승인과 짧은 최종 승인을 유지합니다.

- 검토 시 최신 가격·수량·세션 계산
- 주문 묶음 전체 예상금액을 HWM75·JDSS 현금·토스 주문가능금액으로 사전점검
- 최종 클릭 직전 계좌 대조와 전체 한도 재검사
- 각 종목 제출 직전 가격·수량·세션 재검사
- 승인은 유효시간이 짧고 한 번만 사용 가능
- 같은 버튼을 반복하거나 동시에 눌러도 한 번씩 순서대로 처리
- 중간 실패·결과 확인 필요·거부·취소·가격변경 시 이후 매수 중단
...
| 3차 | 100% | 전량 체결 후 첫 투자 단계 완료 |

`/onboarding`에서 단계 한도를 여는 것과 매수 주문 실행은 별개입니다. 단계가 열리면 그 단계에 필요한 수량이 `오늘 주문 확인`에 나타나고 다시 최종 승인을 거칩니다.

시장 악화로 최종 목표가 낮아지면 첫 투자 단계보다 위험축소 매도가 우선합니다.

## 9. 실거래 원장과 모의운용 원장 구분

현재 Oracle은 실계좌와 연결된 별도 실거래 원장을 사용합니다. 과거 모의운용 원장은 실거래 원장으로 재사용하지 않았습니다.

| 구분 | 역할 |
|---|---|
| 실거래 JDSS 원장 | JDSS가 관리하는 QQQ·TQQQ·SOXL의 현금·주문·체결 기록 |
| 실제 Toss 계좌 | 관리종목 보유·미체결·매수가능금액의 최종 확인 기준 |
| 과거 모의운용 원장 | 시험 기록 전용이며 실거래 수량으로 채택하지 않음 |

실거래에서는 Toss의 관리종목 보유·주문과 JDSS 원장을 계속 대조합니다. 둘이 다르면 성공으로 추정하거나 임의로 수량을 맞추지 않고 신규 매수를 차단합니다.

QQQ·TQQQ·SOXL 외 개인 보유종목은 JDSS 목표비중·투자한도·자동매도·계좌 대조 대상에서 제외됩니다.

## 10. 대표적인 운영 이상상황

| 상황 | 안전한 처리 |
|---|---|
| 주문 응답이 불명확 | 성공으로 추정하거나 재주문하지 않고 `UNKNOWN(결과 확인 필요)`·안전정지 |
| 매도 부분체결·미완료 | 신규 매수 차단 |
| 사용자가 Toss 앱에서 관리종목을 수동 거래 | 계좌 대조에서 불일치하면 매수 차단; 동시 수동거래 자체를 운영상 금지 |
| 검토 후 가격·수량 변경 | 기존 승인 폐기 후 재검토 |
| 여러 매수 중 두 번째 주문 실패 | 첫 주문은 실제 상태로 추적하고 이후 주문 중단 |
| 서버 재시작 | 저장 목표수량·진행 중 주문·원장으로 확인 가능한 차이만 복구 |
| 오래된 Telegram 버튼 | 현재 단계·승인값·유효시간과 다르면 거부 |
...
데이터 공급자의 수정주가 이력이 소폭 바뀔 수 있어 자동검사는 단일 숫자 일치를 강제하지 않고 **CAGR 21.7~22.8%, MDD -31.6~-30.3%, Sharpe 0.94~1.07**의 좁은 재현 범위를 사용합니다. 전략·설정 변경 없이 숫자가 이 범위를 벗어나면 데이터 변화 또는 프로그램 오류를 조사합니다.
...
- 그러나 2023년 이후 데이터는 후보 선택 과정에서 반복 관찰되어 완전히 독립된 검증기간이 아닙니다.
- 과거 조합검증 기준 과최적화 확률 추정치는 약 64.29%로 경고 수준입니다.
- 이 때문에 실제 운영판은 단순성을 우선하고, 새 연구는 [`research/RESEARCH_PROTOCOL.md`](research/RESEARCH_PROTOCOL.md)의 독립검증·인접값·재표본검증 기준을 따릅니다.

## 13. 평소 운영 체크리스트

- 미국장 완결 후: 최신 전략 계산이 끝났는지 확인
- 매수가 없으면 단순히 신호 0건만 보지 말고 `/today`가 표시하는 상태를 확인
- 매수 필요 시: `오늘 주문 확인` → 수량·합계한도 확인 → 순차 실행 최종 승인
- 매도 진행 중이면: 매수를 먼저 시도하지 않고 종료·계좌 대조를 기다림
- 주문 후: `진행 중 주문`과 `/errors`로 부분체결·결과 확인 필요 상태 확인
- 실제 Toss와 과거 모의운용 원장을 같은 것으로 해석하지 않음
- 이상하면 추가 매수보다 안전정지 원인 확인이 먼저
