...
현재 실거래 주문 허용 세션은 **장전·정규장·장후**입니다. 정규장은 유동성과 체결 품질을 위해 허용하며, 모든 세션에서 지정가·2단계 승인·신규 매수 잠금 계약은 동일하게 적용합니다.

`다음 거래세션`은 **신호가 생성된 완결봉과 같은 날의 장후를 뜻하지 않습니다.** 신호 거래일의 다음 미국 거래일 날짜에 도달한 뒤 그 거래일의 장전·정규장·장후 중 허용된 세션에서만 새 목표수량을 고정하고 주문합니다. 이 경계는 현재 백테스트의 `완결봉 신호 → 다음 거래일 체결`과 동일하게 유지합니다.

Telegram `/today`는 주문 허용시간 밖에서 단순 오류를 내는 대신 가능한 경우 **다음 주문 가능 세션을 한국시간으로 안내**합니다. 이 안내는 운영자 UX일 뿐 전략 실행시점을 앞당기지 않습니다.

Toss 현재가 응답은 응답 시각을 필수로 검증합니다. 시각이 없거나 시간대가 없고, 현재 시각보다 2분 이상 미래이거나 5분보다 오래된 가격은 목표수량·지정가 계산에 사용하지 않습니다.
...
SAFE_MODE는 단 한 번의 정상 조회만으로 자동 해제하지 않습니다. 운영자 BUY 잠금이 이미 해제되어 있더라도 SAFE_MODE가 남아 있으면 실제 BUY는 차단 상태입니다.

`/resume`은 운영자 BUY 잠금이 이미 OFF인 경우에도 SAFE_MODE가 남아 있으면 계좌·원장을 다시 대조합니다. 이 재검증 동안에는 로컬 BUY 경계를 임시로 잠가 복구와 신규 주문이 경쟁하지 않게 합니다. 브로커/원장 불일치가 없고, 구버전 직접 포지션 수량과 활성 TP plan이 없는 비활성 SAFE_MODE만 정리할 수 있습니다. 실제 불일치가 발견되면 BUY 잠금을 ON으로 유지하고 SAFE_MODE를 해제하지 않습니다.

Telegram의 `신규 매수` 표시는 최종 주문 가능상태를 반영해야 합니다. `operator_buy_halt=0`이어도 SAFE_MODE가 있으면 `매수 가능`으로 표시하지 않습니다.
