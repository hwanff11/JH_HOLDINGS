...
최초 시작·원금 변경·단계확대는 최종 매수와 같은 잠금 아래 하나의 SQLite 트랜잭션으로 처리합니다. 중첩 저장은 동일 연결과 저장점을 사용하며 내부 조회도 미확정 변경을 일관되게 읽습니다. 실패 시 승인·원금·성과단위·목표·이벤트가 함께 취소됩니다.

확인 버튼에는 설정 수정번호와 검토 당시 주요 운용상태를 연결합니다. 값 변경·되돌림·단계 변경·기한 만료·재사용을 검사합니다. 대표 긴급정지 또는 시스템 임시격리 중 자동 자금확대는 금지합니다.

자동운용 스레드는 최근 동작시각을 DB에 기록합니다. 외부 점검은 실제 배포 코드가 해당 기능을 지원하는지 확인하고, 지원 서버의 초기화 실패·동작시각 누락·시간대 누락·300초 초과 지연을 오류로 처리합니다. 구 배포본에는 기능 미지원으로 표시하며, 신규 배포 후에는 기능 활성화와 실제 갱신을 확인합니다. 실계좌 외부 점검은 토스 토큰을 별도로 발급하지 않습니다.

알림 전송 장애는 주문을 재전송하는 근거가 아닙니다. 전송 실패 기록에는 시각과 오류 종류만 저장하고 토큰·메시지 원문·인증 헤더를 남기지 않습니다.
