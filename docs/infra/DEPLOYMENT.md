...
정기 실거래 배포는 기존·신규 DB 코드가 같아야 합니다. 예외는 PR #334에서 검증한 연결·중첩 트랜잭션 개선의 정확한 이전·신규 파일 해시 쌍뿐입니다. 스키마·초기화·마이그레이션 코드는 동일하며, 다른 파일 내용이나 역방향 교체는 배포 전에 차단합니다. 해당 호환성 검사와 최초 시작 실패복구 테스트는 배포 사전검증에 포함합니다.
