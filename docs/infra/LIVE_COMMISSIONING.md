...
운영 배포 후 신규매수 잠금과 최초 시작 미승인을 유지한 상태에서 다음을 확인합니다.

1. 자동운용 최근 동작시각이 계속 갱신되고 외부 점검이 이를 확인하는지 확인
2. 대시보드의 최근 계좌·원장 대조시각, 알림 실패 기록 및 시작 전 표시 확인
3. 시작 검토에 목표 자동원금과 첫 단계 50% 허용원금이 함께 보이는지 확인
4. 설정 변경 후 이전 확인 버튼을 거부하는지 확인

배포 작업에서 최초 시작 버튼을 대신 누르지 않습니다. 저장 실패는 새 검토로 다시 시도하며, 기존 불완전 회계가 검출되면 자동 초기화나 수동 DB 값 변경으로 우회하지 않고 먼저 원장을 점검합니다.
