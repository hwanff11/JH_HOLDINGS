...
> **한 문장 요약:** QQQ를 기본으로 보유하고 시장이 강할 때만 TQQQ·SOXL로 속도를 높이며, 위험이 커지면 자동으로 줄이고 BUY만 사람이 최종 승인하는 전략입니다.

현재 production 전략은 **`JDSS-3.2.2-RS6M-ONEWAY-HWM75`**입니다. 정확한 숫자와 예외조건은 [`JDSS_FINAL_SPEC.md`](JDSS_FINAL_SPEC.md)와 [`../strategy.yaml`](../strategy.yaml), 현재 배포·live 상태는 [`../CURRENT_WORK.md`](../CURRENT_WORK.md)가 기준입니다.

현재 Oracle은 **forced dry-run**이고 실제 Toss BUY/SELL 주문은 잠겨 있습니다. 아래의 주문 흐름은 현재 모의 주문에 적용됩니다.
...
월중에는 위험이 커지면 빠르게 줄일 수 있지만 같은 달 안에서 다시 공격적으로 올리지 않는 **risk-off fast / risk-on slow** 구조입니다.
...
flowchart TD
    ...
    J --> K{"원장 일치?"}
    K -->|"예"| A
    K -->|"아니오"| L["SAFE_MODE · 신규 BUY 차단"]
```

평소 사용자는 `/dashboard`에서 **`오늘 주문 한번에 검토`**만 기억하면 됩니다.

- SELL이 필요하면 시스템이 먼저 위험을 줄입니다.
- BUY가 필요하면 QQQ/TQQQ/SOXL을 한 화면에 모아 보여줍니다.
- 사용자가 최종 버튼을 한 번 누르면 각 종목을 **순서대로 독립 제출**합니다.
- 순차 실행은 하나의 원자적 basket 주문이 아니므로 중간에 조건이 달라지면 앞 주문만 제출되고 뒤 주문은 중단될 수 있습니다.
- 최종 클릭 직전에 정합성·가격·수량·세션·HWM75 한도를 다시 검사합니다.
...
다음 단계는 자동으로 열리지 않고 운영자가 `/onboarding`에서 승인합니다. 단계만 열어도 주문이 자동 제출되는 것은 아니며 실제 BUY는 다시 `오늘 주문 한번에 검토` 흐름을 거칩니다.

## 7. 기준 백테스트

최근 canonical V3.2.2 재현 결과(2011-01-01~2026-08-24, 초기 $50,000, 매수·매도 수수료 각 0.1%, 슬리피지 0.1%, next-session execution)는 다음과 같습니다.
...
- BUY는 운영자 승인, 중복주문·한도초과·원장 불일치에는 fail-closed로 대응합니다.

### 한계

- 매년 QQQ를 이기는 전략이 아닙니다.
- TQQQ·SOXL은 급락·횡보장에서 손실과 변동성 드래그가 큽니다.
- 2023년 이후 데이터는 연구 과정에서 반복 관찰되어 완전히 깨끗한 OOS로 보기 어렵습니다.
...
| HWM75 | 최고점 이익의 75%만 위험예산 증가에 반영하는 규칙 |
| MDD | 최고점 이후 가장 크게 빠진 비율 |
| Sharpe | 전체 변동성 대비 수익의 위험조정 참고지표 |
| SAFE_MODE | 주문·원장 상태가 불확실할 때 신규 BUY를 막는 보호 상태 |
| reconciliation | 프로그램 원장과 주문/보유 상태가 맞는지 대조하는 절차 |
