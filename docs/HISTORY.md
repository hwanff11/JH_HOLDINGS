...
현재 계약은 [`JDSS_FINAL_SPEC.md`](JDSS_FINAL_SPEC.md)와 [`../strategy.yaml`](../strategy.yaml), 자동운용 계약은 [`JH_AUTO_SPEC.md`](JH_AUTO_SPEC.md), 현재 배포·live 상태는 [`../CURRENT_WORK.md`](../CURRENT_WORK.md)가 기준입니다. 상세 과거 원본은 Git tag, 병합 PR, 연구 PR과 Actions artifact에서 복구합니다.
...
| 위험축소 SELL 자동 / 위험증가 BUY 엄격 검증 | 위험 감소는 지연하지 않고 위험 증가는 더 강한 안전경계 적용 |
| forced dry-run 모의원장과 실제 Toss read-only 조회 분리 | 모의 보유를 실제 계좌 보유로 오인하지 않기 위함 |
| release-local venv + DB snapshot + atomic switch + rollback | 배포 중단점과 DB 복구경계를 명확히 함 |
...
기능은 forced dry-run으로 배포했고 live hard lock을 유지했습니다. 상세 현재 계약은 공식 사양·Telegram 가이드·보안 기준으로 이동했으며 이 역사 문서에는 결정만 남깁니다.
...
일/주/격주/월 리밸런싱과 제한적 mid-month re-risk를 비교했지만 **월간 reset이 비용·MDD·안정성의 균형에서 계속 강했습니다.** 일부 mid-month 후보는 CAGR을 소폭 높였으나 특정 whipsaw 환경에서 약해 production 승격 대신 연구 후보로만 남겼습니다.

핵심 교훈은 “더 자주 판단하면 더 빨리 수익을 회복할 것”이라는 직관이 항상 맞지 않으며, 빠른 re-risk가 오히려 whipsaw drawdown을 키울 수 있다는 점입니다.
...
따라서 production 대체가 아니라 독립 SHADOW 후보로 보관했습니다. 상세 Phase 1~13 수치와 스크립트는 Draft 연구 PR #193과 Actions artifact에 남깁니다.
...
## 2026-09-01 — V3.2.2 유지와 최초진입 방식 결정

당시 2026-09-01 적용 후보를 검토한 결과 **JDSS 3.2.2를 유지**하고 V3.3·Ultra Alpha는 SHADOW 연구로 남겼습니다.

핵심 결정:

- 전략 ID 유지: `JDSS-3.2.2-RS6M-ONEWAY-HWM75`
- 최초 위험개방 방식은 **50% → 75% → 100%**, 단계 간 최소 3 미국 거래세션을 기준으로 채택
- 2011-01-03~2026-02-26의 3,810개 시작점 비교에서 즉시진입보다 초기 하방을 완화하고 더 느린 분할보다 상승장 기회비용을 줄이는 균형안으로 판단
- 월초라는 이유만으로 1거래세션을 추가 대기하는 별도 타이밍 규칙은 채택하지 않음
- Ultra Alpha 전체 조합은 CAGR 개선 대비 MDD·Sharpe·validation 열위 때문에 production 승격하지 않음
- Phase4 `G_CD0_M2_OPEN`도 2018 whipsaw 취약성 때문에 production 승격 보류

당시 문서에는 `2026-09-01`이라는 실행계획 날짜가 포함되어 있었으나 이는 **과거 연구·운영결정의 시점**입니다. 현재 운영지시로 사용하지 않습니다. 상세 표·paired 비교·당시 문구는 Git 이력에서 복구합니다.
...
## 2026-09-03 — 최초 운용 전 안전강화·조회 개선 경과

- PR #316·#318: 실거래 안전경계와 실제 자동운용 메뉴 기준 배포 점검을 정리했습니다.
- PR #321: 동일 원장 중복 실행 방지, 정규장 실행, 자동주문·체결 연결, 단계별 실제 체결 확인을 추가했습니다.
- PR #324: 연구 기준자금과 실제 위임자금, 최초 시작 전 표시, 자동승인 설명을 정리했습니다.
- PR #327·#330: 표시 전용 시세 묶음조회·단기 캐시와 대시보드 직접 조회 경로를 적용했습니다.
- 상세 검증·배포 경과는 각 PR에 보존합니다. 현재 운영 상태와 미배포 변경은 `CURRENT_WORK.md`에서 확인합니다.

## 2026-09-07 — 첫 운용 승인·주문감시 안전강화 운영 반영

- PR #334에서 Telegram 알림 실패와 주문감시를 분리하고, 과거 설정 확인버튼과 부분 저장·중복 승인을 차단했습니다. 긴급정지 중 자금확대를 중지하고 운영 메시지와 현행 문서를 정리했습니다.
- PR #336에서 스키마가 동일한 검증된 연결·트랜잭션 개선만 정확한 파일 해시 쌍으로 허용하도록 배포 호환성을 보완했습니다.
- 실계좌 연결·원장·신규매수 잠금을 보존해 운영 배포하고, 외부 주문감시·DB·서비스 정상 확인까지 완료했습니다. 최초 시작승인·매수 재개는 수행하지 않았습니다.

## 2026-09-07 — 문서 체계 19개 → 13개 통합

현행 Markdown의 중복소유로 인한 drift를 줄이기 위해 문서 구조를 책임 단위로 통합했습니다.

- `ONE_PAGE_REPORT`의 대표용 요약·장단점·용어사전을 `STRATEGY_GUIDE` 상단/하단으로 통합
- Telegram 사용법과 live 메시지 표현기준을 `TELEGRAM_BOT_GUIDE`로 통합
- pre-live hardening의 고유 안전조건을 `JH_AUTO_SPEC`과 기술 보안계약으로 이관
- live commissioning의 실계좌 연결·배포·복구는 `DEPLOYMENT`, 자금·시작은 `JH_AUTO_SPEC`, 운영자 조작은 Telegram, 주문안전은 `infra/SECURITY`로 분해
- 과거 `STRATEGY_FREEZE` 상세는 Git 이력으로 보존하고 위 2026-09-01 결정만 이 역사 색인에 유지
- `docs/README`의 문서 지도·소유권·충돌 판정은 루트 `README`로 통합
- `AGENTS`는 강제 실행규칙, `DEVELOPMENT_WORKFLOW`는 사람 중심 상세 절차로 역할을 분리
- 루트 `SECURITY`는 공개정책, `docs/infra/SECURITY`는 기술 안전불변식으로 역할을 명확화

이 변경은 문서·문서계약 테스트만 변경하며 Oracle runtime 동작을 바꾸지 않습니다.
